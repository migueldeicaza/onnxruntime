...
void usage() {
  printf(
      ...
      "'openvino', 'nuphar', 'migraphx', 'acl', 'mlcompute' or 'armnn'. "
      "Default: 'cpu'.\n"
...
int real_main(int argc, char* argv[], Ort::Env& env) {
      ...
      switch (ch) {
        ...
        case 'e':
          if (!CompareCString(optarg, ORT_TSTR("cpu"))) {
            ...
            enable_armnn = true;
          } else if (!CompareCString(optarg, ORT_TSTR("mlcompute"))) {
            enable_mlcompute = true;
  ...
  std::vector<std::unique_ptr<ITestCase>> owned_tests;
  {
    ...
    if (enable_mlcompute) {
#ifdef USE_MLCOMPUTE
      Ort::ThrowOnError(OrtSessionOptionsAppendExecutionProvider_MLCompute(sf));
#else
      fprintf(stderr, "MLCompute is not supported in this build\n");
      return -1;
#endif
    }
    if (enable_migraphx) {
