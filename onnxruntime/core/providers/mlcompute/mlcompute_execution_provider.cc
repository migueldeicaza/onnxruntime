...
#include "mlcompute_execution_provider.h"
  ...
  AllocatorCreationInfo cpu_memory_info(
      [](int) {
        return onnxruntime::make_unique<CPUAllocator>(
            OrtMemoryInfo(MlCompute, OrtAllocatorType::OrtDeviceAllocator, OrtDevice(), 0, OrtMemTypeCPUOutput));
      });
...
MlComputeExecutionProvider::GetCapability(const onnxruntime::GraphViewer& graph_view,
  ...
  const auto supported_nodes_vector = GetSupportedNodes(graph_view);
  ...
  for (const auto& group : supported_nodes_vector) {
    ...
    LOGS_DEFAULT(VERBOSE) << "MlComputeExecutionProvider::GetCapability, current supported node group size: "
                          << group.size();
    ...
    meta_def->name = "MlCompute_" + std::to_string(metadef_id_++);
  ...
  LOGS_DEFAULT(INFO) << "MlComputeExecutionProvider::GetCapability,"
                     << " number of partitions supported by MlCompute: " << result.size()
                     << " number of nodes in the graph: " << graph_view.NumberOfNodes()
                     << " number of nodes supported by MlCompute: " << num_of_supported_nodes;
...
common::Status MlComputeExecutionProvider::Compile(const std::vector<FusedNodeAndGraph>& fused_nodes_and_graphs,
  ...
  //   std::unique_ptr<MlCompute::Model> MlCompute_model;
  //   ORT_RETURN_IF_ERROR(builder.Compile(MlCompute_model));
  ...
  //     }
  //     MlCompute_model->SetInputMap(std::move(input_map));
  //   }
  ...
  //     }
  //     MlCompute_model->SetOutputMap(std::move(output_map));
  //   }

  //   MlCompute_models_.emplace(fused_node.Name(), std::move(MlCompute_model));

  //   NodeComputeInfo compute_info;
  //   compute_info.create_state_func = [&](ComputeContext* context, FunctionState* state) {
  //     *state = MlCompute_models_[context->node_name].get();
  //     return 0;
  //   };

  //   compute_info.release_state_func = [](FunctionState state) {
  //     // the `state` is a MlCompute::model managed by unique_ptr
  //     ORT_UNUSED_PARAMETER(state);
  //   };

  //   compute_info.compute_func = [](FunctionState state, const OrtCustomOpApi* api, OrtKernelContext* context) {
  //     Ort::CustomOpApi ort{*api};
  //     MlCompute::Model* model = reinterpret_cast<MlCompute::Model*>(state);
  ...
  //     std::vector<MlCompute::Execution::InputBuffer> inputs;
  ...
  //       // since MlCompute will treat empty shape input as dynamic ranking input, (onnx does not support dynamic ranking)
  ...
  //                            << MlCompute::Shape2String(dimensions) << " shape";
  //       }
  ...
  //     {
  //       std::unique_ptr<MlCompute::Execution> execution;
  ...
  //       std::vector<MlCompute::Execution::OutputBuffer> outputs;
  ...
  //           // Since MlCompute use {1} tensor as scalar, if the model output should have empty shape
